...
def compute_fov(chan_freq, antennas):
    '''Find the grid size as set by the field of view of the antenna.'''
    ...
    min_lambda = scipy.constants.c/min_freq
    d_lambda = diameter / min_lambda    
...
def load_ms_file(msfile, fieldid=None, method='physical', ddid=0, chunksize:int=10**7):
    """
    ...
    if method=='physical':
        ...
        bincount = [int(bincount), int(bincount)]
        ...
        binwidth = [int((uvlimit[0][1] - uvlimit[0][0])/bincount[0]), 
